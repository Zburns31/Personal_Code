...
def author():
    return "zburns6"  # replace gburdell3 with your Georgia Tech username.


def gtid():
    return 903658860  # replace with your GT ID number
...
class PageRank:
    ...
    def calculate_node_degree(self):
        ...
        # self.max_node_id = max(self.node_degree.keys())
    ...
    def run_pagerank(self, node_weights, damping_factor=0.85, iterations=10):
        ...
        for it in range(iterations):
            ...
            for source, target in self.read_edge_file(self.edge_file):
                ...
                # At each iteration of edge (node1, node2), add the damping factor +
                # The PR value of the source node / number of outlinks from that node to any other nodes
                new_pr_values[target] += damping_factor * (
                    pr_values[source] / self.node_degree[source]
                )

            # Add first part of the equation to new PR Values for a given target node
            # Represents the probability of jumping to any random node * the node weight for any given node
            # (where node represents the target node)
        ...
        print(
            "Completed {0}/{1} iterations. {2} seconds elapsed.".format(
                it + 1, iterations, time.time() - start_time
            )
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="sample command: python submission.py -i 5 -d 0.85 simplified_pagerank network.tsv"
    )
    parser.add_argument(
        "command",
        help="Sub-command to execute. Can be  simplified_pagerank or personalized_pagerank.",
    )
